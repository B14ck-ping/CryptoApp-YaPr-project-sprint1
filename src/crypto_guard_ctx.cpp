...
private:
    void doCryptFile(std::istream &input, std::ostream &output, std::string_view password, int cryptType);
};

void CryptoGuardCtx::Impl::EncryptFile(std::istream &input, std::ostream &output, std::string_view password) {
    doCryptFile(input, output, password, 1);
}
...
void CryptoGuardCtx::Impl::doCryptFile(std::istream &input, std::ostream &output, std::string_view password,
    ...
    auto params = CryptoGuardCtx::Impl::CreateCipherParamsFromPassword(password);
