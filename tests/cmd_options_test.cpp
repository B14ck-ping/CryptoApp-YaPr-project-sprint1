...
TEST(ProgramOptions, TestInputIsRequired) {
    ...
    CryptoGuard::ProgramOptions po;
    ASSERT_THAT([&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
...
TEST(ProgramOptions, TestUnknownCommand) {
    ...
    CryptoGuard::ProgramOptions po;
    ASSERT_THAT([&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
                testing::ThrowsMessage<std::runtime_error>(testing::HasSubstr("Command is invalid.")));
}
...
TEST(ProgramOptions, TestSkippedEncryptOutputFile) {
    ...
    ASSERT_THAT(
        [&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
        testing::ThrowsMessage<std::runtime_error>(testing::HasSubstr("Output file is required for encrypt command!")));
}
...
TEST(ProgramOptions, TestSkippedDecryptOutputFile) {
    ...
    ASSERT_THAT(
        [&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
        testing::ThrowsMessage<std::runtime_error>(testing::HasSubstr("Output file is required for decrypt command!")));
}

TEST(ProgramOptions, TestSkippedEncryptPassword) {
    static constexpr std::array args = {"CryptoGuard", "--command", "encrypt", "--input",
                                        "123.txt",     "--output",  "456.txt"};

    CryptoGuard::ProgramOptions po;
    ASSERT_THAT(
        [&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
        testing::ThrowsMessage<std::runtime_error>(testing::HasSubstr("Password is required for encrypt command!")));
}

TEST(ProgramOptions, TestSkippedDecryptPassword) {
    static constexpr std::array args = {"CryptoGuard", "--command", "decrypt", "--input",
                                        "123.txt",     "--output",  "456.txt"};

    CryptoGuard::ProgramOptions po;
    ASSERT_THAT(
        [&]() { po.Parse(static_cast<int>(args.size()), args.data()); },
        testing::ThrowsMessage<std::runtime_error>(testing::HasSubstr("Password is required for decrypt command!")));
}

TEST(ProgramOptions, InputAndOutputFilesAreSamePassword) {
    static constexpr std::array args = {"CryptoGuard", "--command", "decrypt", "--input",
                                        "123.txt",     "--output",  "123.txt"};
...
TEST(ProgramOptions, CheckSumUnsupportedCmd) {
    static constexpr std::array args = {"CryptoGuard", "--command", "checksum", "--input",
                                        "123.txt",     "--output",  "123.txt"};
